...
class SignInViewController: UIViewController {
    ...
    @objc private func checkButtonTap() {
        Task {
            let result = try await GetCheckDouble.shared.PostRegisterData(userName: idTextField.text ?? "")
            print(result ?? "")
            print(result?.isExist ?? print("true"))
        }
    }
    ...
    private func setLayout() {
        ...
        [idTextField, passwordTextField, nickNameTextField, registerButton, infoViewButton,doubleCheckButton].forEach {
    ...
    private lazy var doubleCheckButton = UIButton().then {
        $0.addTarget(self,
                     action: #selector(checkButtonTap),
                     for: .touchUpInside)
        $0.backgroundColor = .blue
